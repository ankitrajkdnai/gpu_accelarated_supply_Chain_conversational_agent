...
class GraphAnalytics:
    ...
    def run_centrality_analysis(self, graph: Optional[cugraph.Graph] = None, 
        ...
        try:
            ...
            if method == 'betweenness':
                ...
                result = cugraph.eigenvector_centrality(G, max_iter=2000, tol=1e-4)
    ...
    #         distances = cugraph.bfs(G, source_vertex)
    ...
    def run_shortest_path(self, source_vertex: Any, graph: Optional[cugraph.Graph] = None) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            # Get edge list and check if 'weight' column exists
            edgelist = G.view_edge_list()
            self.logger.debug(f"Edge list columns: {edgelist.columns}")
            self.logger.debug(f"Edge list preview:\n{edgelist.head()}")

            # Rename to match expected format
            edgelist = edgelist.rename(columns={'source': 'src', 'target': 'dst'})

            # Add weights if missing
            if 'weights' not in edgelist.columns:
                self.logger.warning("No weight column found in graph. Assigning default weight = 1.0")
                edgelist['weights'] = 1.0

            G.clear()
            G.from_cudf_edgelist(edgelist, source='src', destination='dst', edge_attr='weights')
            # if 'weights' not in edgelist.columns:
            #     self.logger.warning("No weight column found in graph. Assigning default weight = 1.0")
            #     edgelist['weights'] = 1.0
            #     G.clear()
