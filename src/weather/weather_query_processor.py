...
class WeatherQueryProcessor:
    ...
    def _extend_schema_context(self):
        ...
        - supplier_affected_by_weather: suppliers → weather_events
        """
        self.query_processor.schema_context += "\n" + weather_schema_context
...
# Example Integration in the main QueryProcessor:
...
class QueryProcessor:
    ...
    def generate_aql(self, question: str) -> str:
        if self.weather_processor.detect_weather_query(question):
            # Example mapping - you can implement a proper parser or ML model
            if "alternative" in question:
                return self._query_templates["alternative_suppliers_for_weather_affected"].format(impact_threshold=50)
            elif "affected suppliers" in question or "suppliers affected" in question:
                return self._query_templates["suppliers_affected_by_weather"].format(min_severity=3, limit=10)
            elif "weather events" in question or "weather in" in question:
                return self._query_templates["weather_events_by_country"].format(country="US", min_severity=2, limit=10)
    #    return "// Fallback to default template processing logic"
