...
# Set up Python path (only once)
unset PYTHONPATH  # Clear any existing PYTHONPATH first
export PYTHONPATH="$PROJECT_ROOT"
echo "PYTHONPATH set to: $PYTHONPATH"
...
if [[ -z "$VIRTUAL_ENV" ]]; then
    if [ -d "$PROJECT_ROOT/venv" ]; then
        ...
        echo "Please create a virtual environment: python -m venv
$PROJECT_ROOT/venv"
        exit 1
    fi
else
...
# Ask for confirmation before proceeding
read -p "This will clear all existing data in ArangoDB. Do you want to
continue? (y/n) " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    echo "Operation cancelled"
...
echo -e "\n=== Running Data Processing Pipeline ==="
echo "Processing raw data files..."
PYTHONPATH="$PROJECT_ROOT" python "$SCRIPT_DIR/process_data.py"
...
if [ $? -eq 0 ]; then
    ...
    echo -e "\n=== Running Data Ingestion Pipeline ==="
    echo "Ingesting processed data into ArangoDB..."
    PYTHONPATH="$PROJECT_ROOT" python "$SCRIPT_DIR/ingest_data.py"
    ...
    if [ $? -eq 0 ]; then
        ...
        echo -e "\n=== Running Data Validation ==="
        echo "Validating data in ArangoDB..."
        PYTHONPATH="$PROJECT_ROOT" python "$SCRIPT_DIR/validate_data.py"
