...
VENV_PATH="$PROJECT_ROOT/venv"  # Adjust if your venv is elsewhere
if [ -d "$VENV_PATH" ]; then
    echo "Activating virtual environment..."
    source "$VENV_PATH/bin/activate"  # For macOS/Linux
    # source "$VENV_PATH/Scripts/activate"  # For Windows (if applicable)
else
    echo "Virtual environment not found at $VENV_PATH. Please create and activate it manually."
    exit 1
fi
